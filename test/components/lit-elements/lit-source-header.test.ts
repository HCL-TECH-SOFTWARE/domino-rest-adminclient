...
import '../../../src/components/lit-elements/lit-source-header';
import type SourceContents from '../../../src/components/lit-elements/lit-source-header';

const TAG = 'lit-source';
const q = (el: SourceContents, sel: string) => el.shadowRoot!.querySelector(sel);

describe('lit-source-header (tag: lit-source)', () => {
  afterEach(cleanupLit);

  it('registers under the lit-source tag', () => {
  ...
  it('renders the tree editor only in tree view', async () => {
    ...
    expect(q(text, 'lit-source-tree')).toBeNull();

    const tree = await mountLit<SourceContents>(TAG, { selectedOption: 'tree', content: { a: 1 } });
    expect(q(tree, 'lit-source-tree')).toBeTruthy();
  });
