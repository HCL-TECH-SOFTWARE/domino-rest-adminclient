...
import '../../../src/components/lit-elements/lit-textform-array';
import type TextFormArray from '../../../src/components/lit-elements/lit-textform-array';

const TAG = 'lit-textform-array';
...
describe('lit-textform-array', () => {
  ...
  it('renders a wa-details with a lit-textform per data entry', async () => {
    const el = await mountLit<TextFormArray>(TAG, { data: rules(), title: 'formula' });
    expect(qa(el, 'wa-details').length).toBe(2);
    expect(qa(el, 'wa-details lit-textform').length).toBe(2);
  });
  ...
  it('opens the delete dialog and shows the rule message when Delete Rule is clicked', async () => {
    ...
    expect(showModal).toHaveBeenCalled();
    expect(q(el, 'dialog#delete lit-dialog-content')!.textContent).toContain('Msg1');
  });
  ...
  it('pushes edits up through setData on data-changed', async () => {
    ...
    const firstForm = qa(el, 'wa-details lit-textform')[0];
  ...
  it('removes the selected rule and calls setData when delete is confirmed', async () => {
    ...
    (q(el, 'dialog#delete lit-button-yes') as HTMLElement).click(); // confirm
