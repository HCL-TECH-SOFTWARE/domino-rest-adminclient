...
describe('app-icons service', () => {
  ...
  /*
   * A `describe('the subscription seam')` stood here, driving `useAppIcons()` through
   * `renderHook`. Both are gone in #806 wave 5, and the order matters:
   *
   * The hook was added to *this* file one wave earlier, when converting `AppIcon` deleted the
   * component test that had been its only cover and dropped `src/services/**` under its gate.
   * Wave 5 then converted the hook's last two callers — `ScopeFormContainer` and
   * `DetailsSection` — so the hook itself became unreachable and was deleted, taking the
   * subscriber set and the last `from 'react'` in `src/services/` with it.
   *
   * So these assertions are not being dropped to make a gate go green; the code they exercised
   * no longer exists. What replaced it is the promise every converted element already awaits —
   * `loadAppIcons()` — which the tests above cover directly, including the concurrent-caller
   * and cached-resolution paths that the subscribe path used to reach incidentally.
   */
});
