 ...
 *
 * ## Raw was 3 % for the duration of #806, and is back to 2 %
 *
 * Raw was the metric that migration pushed on, for reasons that had nothing to do with a
 * regression: converting a React view to a Lit element moves its markup from JSX into a
 * `static styles` + `html` template, and #718 inlined 44 icon glyphs as base64 `data:` URIs.
 * Both add *raw* bytes that gzip absorbs almost completely — the icon codemod landed at
 * **+19.5 kB raw but only +4.9 kB gzip**. A raw budget tight enough to catch a real
 * regression during a migration that legitimately adds raw bytes just fails on the migration.
 *
 * gzip stayed at 2 % throughout, precisely because it was the metric that did *not* move for
 * those reasons, so it remained the sensitive half of the gate.
 *
 * The note here said to put raw back "when #806 closes", and #719 half 2 is where that
 * happens: there is no `.tsx` under `src` any more, so no view can convert and add raw bytes
 * again. Re-baselined in the same change, as that note also required.
 */
const HEADROOM = { raw: 0.02, gzip: 0.02 };
