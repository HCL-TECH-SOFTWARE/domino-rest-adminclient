...
/**
 * Key/value editor for a single formula rule. Tag: `lit-textform`.
 * Emits a (non-bubbling) `data-changed` CustomEvent with the updated data.
 */
@customElement('lit-textform')
export default class TextForm extends KeepLitElement {
  static styles = css`
    .row {
      display: flex;
      align-items: center;
        ...
        (key) => html`
          <div class="row">
            ...
            <div class="value">
              ${key === 'formulaType'
                ? html`
                  <lit-autocomplete .options=${['domino']} .initialOption="${this.data[key]}" @input=${(event: Event) => this.handleInputChange(key, event)}></lit-autocomplete>`
                : html`
